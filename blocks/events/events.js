...
export default async function decorate(block) {
  ...
  index.data.eventList.items.forEach((item) => {
    itemsHTML += `
    <li data-aue-resource="urn:aemconnection:${item._path}/jcr:content/data/master" data-aue-type="reference" data-aue-label="events-li content fragment" data-aue-filter="cf" class="events">
        <div class="events-image">
      		<picture>
            <img loading="lazy" alt="" src="${aempublishurl + item.picture._dynamicUrl}" data-aue-prop="picture" data-aue-label="Picture" data-aue-type="media" class='events-picture'>
          </picture>
      	</div>
        <div class="events-body">
            <div data-aue-prop="location" data-aue-label="Location" data-aue-filter="text" data-aue-type="text" class="events-location">${item.location[0]}</div>
    			  <div data-aue-prop="title" data-aue-label="Title" data-aue-filter="text" data-aue-type="text" class="events-title">${item.title}</div>
            <div data-aue-prop="ctaText" data-aue-label="Call to action" data-aue-filter="text" data-aue-type="text" class="events-cta">${item.ctaText}</div>
    	  </div>
      </li>`;
  });
