...
export default async function decorate(block) {
  ...
  index.data.faqList.items.forEach((item) => {
    itemsHTML += `
    <li data-aue-resource="urn:aemconnection:${item._path}/jcr:content/data/master" data-aue-type="reference" data-aue-label="faqs content fragment" data-aue-filter="cf">
        <details class="faqs-details">    
          <summary class="faqs-heading">
            <span data-aue-prop="question" data-aue-label="question" data-aue-type="text">${item.question}</span>
            <b></b>
          </summary>
          <div data-aue-prop="answer" data-aue-label="answer" data-aue-type="richtext" class="faqs-description">${item.answer.html}</div>
        </details>
      </li>`;
  });
