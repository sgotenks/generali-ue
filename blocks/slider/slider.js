  ...
  if (interval > 0) {
    block.dataset.intervalIdPercentage = setInterval(() => {
      ...
      const indicator = block.querySelector(
        `.slider-slide-indicator[data-target-slide="${slideIndex}"] > button`,
      );
      if (!indicator) return;
      const percentage = (1 + block.loadPercentage) % 100;
      block.loadPercentage = percentage;
      indicator.style.width = `${percentage}%`;
      block.querySelectorAll('.slider-slide-indicator > button').forEach((ind) => {
...
function updateActiveSlide(slide) {
  ...
  const indicators = block.querySelectorAll('.slider-slide-indicator');
...
export function showSlide(block, slideIndex = 0) {
  ...
  block.querySelectorAll('.slider-slide-indicator > button').forEach((ind, index) => {
...
function bindEvents(block) {
  ...
  block.querySelectorAll('.slider-slide').forEach((slide) => {
...
function createSlide(row, slideIndex, carouselId) {
  ...
  slide.setAttribute('id', `slider-${carouselId}-slide-${slideIndex}`);
  slide.classList.add('slider-slide');

  row.querySelectorAll(':scope > div').forEach((column, colIdx) => {
    column.classList.add(`slider-slide-${colIdx === 0 ? 'image' : 'content'}`);
    slide.append(column);
  });
...
export default async function decorate(block) {
  carouselId += 1;
  block.setAttribute('id', `slider-${carouselId}`);
  ...
  block.setAttribute('aria-roledescription', placeholders.carousel || 'Slider');
  ...
  if (!isSingleSlide) {
    ...
    slideIndicatorsNav.setAttribute(
      'aria-label',
      placeholders.carouselSlideControls || 'slider Slide Controls',
    );
